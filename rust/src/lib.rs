...
use protobuf::well_known_types::struct_::Struct;
use protobuf_json_mapping::{parse_from_str, print_to_string, PrintError};
...
pub fn to_struct(proto_msg: &dyn MessageDyn) -> Result<Struct, PrintError> {
    ...
    let msg_struct = parse_from_str::<Struct>(&msg_json)
	.map_err(|_| PrintError::from(std::fmt::Error))?;
